...
const schema=z.object({customerName:z.string().trim().min(2,'Enter your full name.').max(100),phone:z.string().trim().regex(/^[+\d][\d\s-]{8,14}$/,'Enter a valid mobile number.'),whatsapp:z.string().trim().regex(/^$|^[+\d][\d\s-]{8,14}$/,'Enter a valid WhatsApp number or leave it blank.').optional(),email:z.string().trim().email('Enter a valid email or leave it blank.').or(z.literal('')).optional(),address1:z.string().trim().min(5,'Enter your delivery address.').max(180),address2:z.string().trim().max(180).optional(),city:z.string().trim().min(2,'Enter your city.').max(80),district:z.enum(districts,{errorMap:()=>({message:'Choose your district.'})}),postalCode:z.string().trim().regex(/^$|^\d{5}$/,'Postal code must be 5 digits or blank.').optional(),deliveryNotes:z.string().trim().max(300).optional(),paymentMethod:z.enum(['cod','bank']),paymentReference:z.string().trim().max(100).optional(),paymentReceiptUrl:z.string().url().optional()});
...
export default function CheckoutPage(){
 ...
 if(!cart.items.length||cart.items.some(item=>item.isPreorder))return <Navigate to="/cart" replace/>;
 ...
 async function submit(event:FormEvent<HTMLFormElement>){
  ...
  for(const item of cart.items){const product=data.products.find(p=>p.id===item.productId&&p.status==='published'),variant=product?.variants.find(v=>v.id===item.variantId&&v.active);if(!product||!variant||variant.stock<item.quantity){setError(item.name+' is no longer available in the selected quantity. Please review your bag.');return}}
  ...
  if(!parsed.success){setError(parsed.error.issues[0]?.message||'Please review the required information and try again.');return}
  setBusy(true);
 ...
 return <main className="container py-10 lg:py-16"><Seo title="Checkout"/>{user?<p className="mb-7 border-l-2 border-bronze bg-white/30 px-4 py-3 text-xs text-ink/60">Signed in as <b className="text-ink">{user.email}</b>. This order will be saved to <b className="text-ink">My Orders</b>.</p>:<p className="mb-7 border-l-2 border-black/15 bg-white/20 px-4 py-3 text-xs text-ink/55">Checking out as a guest. <a href="/account/login" className="underline underline-offset-4">Sign in first</a> if you want this order saved to My Orders.</p>}<form onSubmit={submit} className="grid gap-12 lg:grid-cols-[1fr_420px]"><div className="space-y-12">
  <section><p className="eyebrow mb-5">01 · Contact</p><div className="grid gap-4 sm:grid-cols-2"><Field name="customerName" label="Full Name *" required autoComplete="name" wide defaultValue={savedName}/><Field name="phone" label="Mobile Number *" required type="tel" autoComplete="tel" defaultValue={savedMobile}/><Field name="whatsapp" label="WhatsApp Number" type="tel" defaultValue={savedMobile}/><Field name="email" label={user?'Email (account email)':'Email (optional)'} type="email" autoComplete="email" wide defaultValue={user?.email||''} readOnly={Boolean(user)}/></div></section>
