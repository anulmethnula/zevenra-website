...
export default function CheckoutPage(){
 ...
 </div><aside className="h-fit bg-[#e7e1d7] p-6 lg:sticky lg:top-6"><p className="eyebrow">Order summary</p><div className="mt-6 space-y-4">{cart.items.map(item=><div key={item.variantId} className="flex gap-3"><div className="relative">{item.image?<img src={item.image} alt={item.name} className="h-20 w-14 object-cover"/>:<div className="h-20 w-14 bg-black/5"/>}<span className="absolute -right-2 -top-2 grid h-5 w-5 place-items-center rounded-full bg-ink text-[10px] text-white">{item.quantity}</span></div><div className="flex-1 text-sm"><p>{item.name}</p><p className="mt-1 text-xs text-ink/50">{item.color} / {item.size}</p></div><span className="text-sm">{money(currentPrice(item)*item.quantity)}</span></div>)}</div><div className="my-6 border-t hairline"/><div className="space-y-3 text-sm"><div className="flex justify-between"><span>Subtotal</span><span>{money(displaySubtotal)}</span></div><div className="flex justify-between"><span>Delivery</span><span>{fee?money(fee):'Complimentary'}</span></div><div className="flex justify-between border-t hairline pt-4 text-base"><span>Total</span><span>{money(displaySubtotal+fee)}</span></div></div>{payment==='bank'&&<div className="mt-5 flex items-start gap-2 bg-white/35 p-3 text-xs leading-5 text-ink/60"><FileText size={16} className="mt-0.5 shrink-0"/><span>Your bank receipt must finish uploading before the order can be placed.</span></div>}{error&&<p role="alert" className="mt-5 bg-red-950 p-3 text-xs leading-5 text-white">{error}</p>}<button disabled={busy||receiptBusy||!availablePayments.length||!settings.storeOpen||!settings.ordersEnabled||(payment==='bank'&&!receiptUrl)} className="btn btn-dark mt-6 w-full disabled:opacity-50">{busy?'Placing order…':receiptBusy?'Uploading receipt…':'Place order'}</button><p className="mt-4 text-center text-[10px] leading-5 text-ink/50">By placing an order, you agree to the store terms. This is an order request, not automatic confirmation.</p></aside></form></main>
}
