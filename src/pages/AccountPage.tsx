...
export default function AccountPage(){
 ...
 const firstName=String(user.user_metadata?.first_name||'').trim()||user.email?.split('@')[0]||'there';
 async function logout(){await signOut();navigate('/')}
 return <main className="account-page"><Seo title="My Account"/><div className="container account-page__inner"><p className="eyebrow text-bronze">Private account</p><h1 className="display">Welcome, <em>{firstName}</em></h1><p className="account-page__lead">Your ZEVENRA orders and account details, quietly gathered in one place.</p><div className="account-grid"><Link to="/account/orders" className="account-card"><Package/><span><small>Order history</small><b>My orders</b></span><ArrowRight/></Link><section className="account-card account-card--details"><UserRound/><span><small>Account details</small><b>{user.user_metadata?.first_name} {user.user_metadata?.last_name}</b><p>{user.email}</p>{user.user_metadata?.mobile&&<p>{user.user_metadata.mobile}</p>}</span></section><button className="account-card account-card--signout" onClick={()=>void logout()}><LogOut/><span><small>Securely end session</small><b>Sign out</b></span><ArrowRight/></button></div></div></main>
}
