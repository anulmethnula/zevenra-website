  ...
  <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
   <div className="space-y-6">
    ...
    <Card title="Size, colour & stock">
     ...
     <div className="grid gap-3 md:hidden">{product.variants.map(v=><div key={v.id} className="border border-black/10 bg-white/45 p-4"><div className="grid grid-cols-2 gap-3"><Field label="Colour" value={v.color} set={value=>variant(v.id,{color:value})}/><Field label="Size" value={v.size} set={value=>variant(v.id,{size:value})}/><Field label="SKU" value={v.sku} set={value=>variant(v.id,{sku:value})}/><label className="block text-xs">Stock<input className="field mt-2" type="number" min="0" value={v.stock} onChange={e=>variant(v.id,{stock:Math.max(0,Number(e.target.value))})}/></label><label className="block text-xs">Low stock alert<input className="field mt-2" type="number" min="0" value={v.lowStockThreshold} onChange={e=>variant(v.id,{lowStockThreshold:Math.max(0,Number(e.target.value))})}/></label><label className="flex min-h-11 items-end gap-2 pb-2 text-xs"><input type="checkbox" className="h-5 w-5 accent-black" checked={v.active} onChange={e=>variant(v.id,{active:e.target.checked})}/> Active</label></div><div className="mt-3 flex justify-end gap-2"><button type="button" className="btn" onClick={()=>set('variants',[...product.variants,{...v,id:crypto.randomUUID(),sku:v.sku?v.sku+'-COPY':''}])}><Copy size={14}/> Duplicate</button><button type="button" className="btn border-red-800 text-red-800" onClick={()=>set('variants',product.variants.filter(x=>x.id!==v.id))}><Trash2 size={14}/> Remove</button></div></div>)}</div>
     <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[780px] text-sm"><thead><tr>{['Colour','Size','SKU','Stock','Low stock','Active',''].map(x=><th key={x} className="p-2 text-left text-xs font-normal">{x}</th>)}</tr></thead><tbody>{product.variants.map(v=><tr key={v.id} className="border-t border-black/10"><td><input className="field my-2" value={v.color} placeholder="Black" onChange={e=>variant(v.id,{color:e.target.value})}/></td><td><input className="field" value={v.size} placeholder="S" onChange={e=>variant(v.id,{size:e.target.value})}/></td><td><input className="field" value={v.sku} placeholder="Optional" onChange={e=>variant(v.id,{sku:e.target.value})}/></td><td><input className="field w-24" type="number" min="0" value={v.stock} onChange={e=>variant(v.id,{stock:Math.max(0,Number(e.target.value))})}/></td><td><input className="field w-24" type="number" min="0" value={v.lowStockThreshold} onChange={e=>variant(v.id,{lowStockThreshold:Math.max(0,Number(e.target.value))})}/></td><td><input type="checkbox" checked={v.active} onChange={e=>variant(v.id,{active:e.target.checked})}/></td><td><div className="flex"><button type="button" title="Duplicate variant" onClick={()=>set('variants',[...product.variants,{...v,id:crypto.randomUUID(),sku:v.sku?v.sku+'-COPY':''}])}><Copy size={15}/></button><button type="button" title="Remove variant" onClick={()=>set('variants',product.variants.filter(x=>x.id!==v.id))} className="ml-3 text-red-800"><Trash2 size={16}/></button></div></td></tr>)}</tbody></table></div>
