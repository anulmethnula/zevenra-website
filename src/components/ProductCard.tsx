...
export function ProductCard({product}:{product:Product}){
 const active=product.variants.filter(variant=>variant.active),sold=active.length===0||active.every(variant=>variant.stock<1),low=!sold&&active.some(variant=>variant.stock>0&&variant.stock<=variant.lowStockThreshold),badge=sold?'SOLD OUT':low?'LOW STOCK':product.newArrival?'NEW':null,images=product.media.filter(media=>media.type==='image'),primary=images[0],secondary=images[1];
