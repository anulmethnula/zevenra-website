...
export type CustomerUser={id:string;firstName:string;lastName:string;email:string;mobile:string;address1:string;address2:string;city:string;district:string;postalCode:string};
type Registration={firstName:string;lastName:string;email:string;password:string;mobile?:string};
export type CustomerProfileUpdate={firstName:string;lastName:string;mobile?:string;address1?:string;address2?:string;city?:string;district?:string;postalCode?:string};
type CustomerAuth={user:CustomerUser|null;loading:boolean;signIn:(email:string,password:string)=>Promise<void>;register:(data:Registration)=>Promise<void>;updateProfile:(data:CustomerProfileUpdate)=>Promise<void>;signOut:()=>Promise<void>;refresh:()=>Promise<void>};
...
export function CustomerAuthProvider({children}:{children:ReactNode}){
 ...
 const updateProfile=useCallback(async(data:CustomerProfileUpdate)=>{const result=await accountRequest<{user:CustomerUser}>('profile',{method:'POST',body:JSON.stringify(data)});setUser(result.user)},[]);
 const signOut=useCallback(async()=>{await accountRequest<{ok:boolean}>('logout',{method:'POST'});setUser(null)},[]);
 const value=useMemo(()=>({user,loading,signIn,register,updateProfile,signOut,refresh}),[user,loading,signIn,register,updateProfile,signOut,refresh]);
