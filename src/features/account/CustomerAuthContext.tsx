...
type CustomerAuth={session:Session|null;user:User|null;loading:boolean;configured:boolean;signIn:(email:string,password:string)=>Promise<void>;register:(data:Registration)=>Promise<boolean>;sendReset:(email:string)=>Promise<void>;signOut:()=>Promise<void>};
const Context=createContext<CustomerAuth|null>(null);

export function CustomerAuthProvider({children}:{children:ReactNode}){
 const[session,setSession]=useState<Session|null>(null),[loading,setLoading]=useState(customerAuthConfigured);
 useEffect(()=>{if(!supabase){setLoading(false);return}let active=true;void supabase.auth.getSession().then(({data})=>{if(active){setSession(data.session);setLoading(false)}});const{data:{subscription}}=supabase.auth.onAuthStateChange((_event,next)=>{setSession(next);setLoading(false)});return()=>{active=false;subscription.unsubscribe()}},[]);
 ...
 const value=useMemo(()=>({session,user:session?.user||null,loading,configured:customerAuthConfigured,signIn,register,sendReset,signOut}),[session,loading,signIn,register,sendReset,signOut]);
