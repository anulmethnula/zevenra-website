...
function DesktopNavigation({items}:{items:NavEntry[]}){return <nav className="desktop-nav" aria-label="Primary navigation">{items.map(item=>item.children?.length?<div className="desktop-category" key={item.id}><NavEntryLink item={item}>{item.label}<ChevronDown size={12}/></NavEntryLink><div className="category-menu category-menu--visual"><div className="category-menu__top"><span>SHOP {item.label}</span><Link to={item.to}>VIEW ALL</Link></div><div className="category-menu__grid"><Link to={item.to} className="category-menu__tile category-menu__tile--all"><span className="category-menu__all-mark">ALL</span><b>ALL {item.label}</b></Link>{item.children.map(child=><Link key={child.id} to={'/category/'+child.slug} className="category-menu__tile">{child.imageUrl?<img src={child.imageUrl} alt="" loading="lazy"/>:<span className="category-menu__placeholder"/>}<b>{child.name.toUpperCase()}</b></Link>)}</div></div></div>:<NavEntryLink key={item.id} item={item}>{item.label}</NavEntryLink>)}</nav>}
