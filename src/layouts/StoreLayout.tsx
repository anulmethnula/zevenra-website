...
export function StoreLayout(){
 ...
 const configuredEntries:NavEntry[]=configured.map(item=>{const href=navigationHref(item,data.categories,data.collections),raw=item.target.replace(/^\/category\//,'');const category=item.linkType==='category'?categories.find(c=>c.id===raw||c.slug===raw):undefined;return{id:item.id,label:item.label.toUpperCase(),to:href,children:category?.children,external:item.linkType==='url'&&/^https?:\/\//i.test(href)}});
 const representedCategories=new Set(configured.filter(item=>item.linkType==='category').map(item=>{const raw=item.target.replace(/^\/category\//,'');return categories.find(c=>c.id===raw||c.slug===raw)?.id}).filter(Boolean));
...
function NavEntryLink({item,children,className}:{item:NavEntry;children:React.ReactNode;className?:string}){return item.external?<a href={item.to} target="_blank" rel="noreferrer noopener" className={className}>{children}</a>:<NavLink to={item.to} className={className}>{children}</NavLink>}
