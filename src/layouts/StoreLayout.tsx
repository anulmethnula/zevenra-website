...
export function StoreLayout(){
 ...
 return <div>{data.settings.announcement&&<div className="announcement">{data.settings.announcement}</div>}<header className={'site-header '+(solid?'site-header--solid':'site-header--hero')}><div className="site-header__inner"><button onClick={()=>setMenu(true)} aria-label="Open menu" className="nav-icon lg:hidden"><Menu size={20}/></button><Link to="/" className="site-logo" aria-label="ZEVENRA home"><img src="/brand/zevenraname.png" alt="ZEVENRA"/></Link><DesktopNavigation items={nav}/><div className="header-actions"><Link to="/shop?focusSearch=1" aria-label="Search products" className="nav-icon hidden sm:flex"><Search size={19}/></Link><Link to={user?'/account':'/account/login'} aria-label={user?'Your account':'Sign in or create account'} className={'nav-icon hidden sm:flex '+(user?'nav-icon--active':'')}><UserRound size={19}/></Link><button onClick={()=>cart.setOpen(true)} aria-label={'Bag with '+cart.count+' items'} className="nav-icon"><ShoppingBag size={19}/><span className="bag-count">{cart.count}</span></button></div></div></header><AnimatePresence>{menu&&<MobileMenu items={nav} close={()=>setMenu(false)} logo="/brand/zevenraname.png" instagram={instagram} tiktok={tiktok} accountHref={user?'/account':'/account/login'}/>}</AnimatePresence><main><Outlet/></main><Footer/><CartDrawer/></div>
}
