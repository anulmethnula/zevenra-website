...
export default async function handler(req:VercelRequest,res:VercelResponse){
 ...
 try{
  ...
  if(/online ordering is temporarily unavailable/i.test(message))return json(res,{error:'Pre-order requests are temporarily paused.'},503);
  if(/already have an active pre-order/i.test(message))return json(res,{error:message},409);
